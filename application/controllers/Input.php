...
class Input extends CI_Controller {
    ...
    public function index() {
        ...
        $data['penghasilan'] = $this->db->order_by("id", "ASC")->get("penghasilan")->result();
        $data['pendidikan'] = $this->db->order_by("id", "ASC")->get("pendidikan")->result();
        $data['subkelas'] = $this->db->order_by("id", "ASC")->get("subkelas")->result();
        ...
        if (isset($_POST['submitsiswa'])) {
            ...
            if ($this->db->get_where("siswadatainduk", ["id" => $idsiswa])->num_rows()) {
            ...
            } elseif (is_null($namalengkap) || is_null($jeniskelamin) || is_null($tempatlahir) || is_null($tanggallahir) || $namalengkap === "" || $jeniskelamin === "" || $tempatlahir === "" || $tanggallahir === "") {
                $data["error"] = '<label style="background-color: red">Kolom nama lengkap, jenis kelamin, tempat lahir dan tanggal lahir wajib diisi semua</label>';
                $this->load->view('input_siswa/inputsiswa', $data);
            } else {
                $this->db->insert("siswadatainduk", [
                    ...
                    "smt" => (is_null($smt) || $smt === "") ? NULL : $smt]);
                $data['namasiswa'] = $namalengkap;
                $data['idsiswa'] = $idsiswa;
                $this->load->view('input_siswa/alamatsiswa', $data);
            }
            ...
            $data['namasiswa'] = $namasiswa;
            $data['idsiswa'] = $idsiswa;
            $this->load->view('input_siswa/telepon', $data);
            ...
            $data['namasiswa'] = $namasiswa;
            $data['idsiswa'] = $idsiswa;
            $this->load->view('input_siswa/hpayah', $data);
